...
# VPN Script Manager for VLESS (WebSocket + TLS) with CDN
# Author: VPN Manager
# Version: 1.0
...
install_dependencies() {
    ...
    apt update
    apt install -y curl wget unzip jq uuid-runtime net-tools iptables-persistent
...
create_client() {
    ...
    local domain=$(jq -r '.domain' "$CONFIG_FILE")
    local port=$(jq -r '.port' "$CONFIG_FILE")
    local vless_uuid=$(jq -r '.vless_uuid' "$CONFIG_FILE")
...
monitor_connections() {
    print_status "Starting connection monitor..."
    
    # Create monitoring script
    cat > /usr/local/bin/vpn_manager/monitor.js << 'EOF'
const fs = require('fs');
const { exec } = require('child_process');

const CLIENTS_FILE = '/etc/vpn_manager/clients.json';
const LOG_FILE = '/var/log/vpn_manager.log';
...
function updateClientUsage(clientName) {
    try {
        const clientsData = JSON.parse(fs.readFileSync(CLIENTS_FILE, 'utf8'));
        const client = clientsData.clients.find(c => c.name === clientName);
        
        if (client) {
            client.last_used = new Date().toISOString();
            client.connection_count = (client.connection_count || 0) + 1;
            fs.writeFileSync(CLIENTS_FILE, JSON.stringify(clientsData, null, 2));
            log(`Client connected: ${clientName}`);
        }
    } catch (error) {
        log(`Error updating client usage: ${error.message}`);
    }
}

function monitorXrayLogs() {
    exec('tail -f /var/log/xray/access.log', (error, stdout, stderr) => {
        if (error) {
            log(`Error monitoring logs: ${error.message}`);
            return;
        }
        ...
        stdout.on('data', (data) => {
            ...
            lines.forEach(line => {
                if (line.includes('accepted')) {
                    // Extract client info from log line
                    const match = line.match(/email=([^\s]+)/);
                    if (match) {
                        const clientName = match[1];
                        updateClientUsage(clientName);
                    }
                }
            });
        });
...
EOF
    ...
    # Start monitoring
    nohup node /usr/local/bin/vpn_manager/monitor.js > /dev/null 2>&1 &
    
    print_success "Connection monitor started"
...
show_menu() {
    clear
    echo -e "${CYAN}================================${NC}"
    echo -e "${CYAN}    VPN Manager for VLESS       ${NC}"
    echo -e "${CYAN}================================${NC}"
    echo
    ...
    echo "7.  Show Live Connections"
    echo "8.  Show Statistics"
    echo "9.  Start Connection Monitor"
